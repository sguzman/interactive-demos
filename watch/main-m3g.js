...
// -----------------------------------------------------------------------------
// Simulation presentation / geometry diagnostics
// -----------------------------------------------------------------------------
...
stackGuides?.addEventListener('change', () => {
  if (trainGeometry.stackGuides) trainGeometry.stackGuides.visible = stackGuides.checked;
});
endshakeGuides?.addEventListener('change', () => {
  if (trainGeometry.endshakeGuides) trainGeometry.endshakeGuides.visible = endshakeGuides.checked;
});
bodyGapGuides?.addEventListener('change', () => {
  if (trainGeometry.bodyGapGuides) trainGeometry.bodyGapGuides.visible = bodyGapGuides.checked;
});
endshakeScale?.addEventListener('change', () => {
  endshakeExaggeration = Number(endshakeScale.value);
});
...
for (const input of [cameraIntensity, lightAzimuth, lightElevation, lightDistance, lightIntensity, ambient, exposure]) {
...
// -----------------------------------------------------------------------------
// Kinematics. M3g keeps the reference ratio graph and train phases while adding
// optional, explicitly exaggerated axial endshake for bearing-stack inspection.
// -----------------------------------------------------------------------------
...
function animate() {
  ...
  // Winding remains presentational until M4 connects crown state to barrel energy.
