...
function animate() {
  ...
  // Apply user winding/setting input first. If that introduces stored energy,
  // the same frame can begin releasing power into the movement.
  windingSystem.update(dt);
  keylessSystem.update(dt);
  powerSystem.advance(dt, trainTimeScale);
  windingSystem.update(0); // refresh reserve readout/energy spring after discharge

  const mechanicalElapsed = powerSystem.state.mechanicalElapsedSeconds;
  const k = movementAngles(mechanicalElapsed);
  const balanceAngle = Math.sin(mechanicalElapsed * Math.PI * 2 * MODEL.frequencyHz) * .43;
